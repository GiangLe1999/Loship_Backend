...
export class OrdersResolver {
  ...
  @Subscription(() => Order, {
    filter: (
      payload: { orderUpdates: Order },
      variables: { input: OrderUpdatesInput },
      context: { user: User },
    ) => {
      const order = payload.orderUpdates;
      const user = context.user;

      // Check that only people who are related to order can make the subscription
      if (
